/**
 * Browser-safe platform bridge for `@deepseek-ai/dsh-subagent-claude-code`.
 *
 * The package historically read `process.platform` (a Node-only global) to
 * decide whether to wrap the native Claude Code CLI in `cmd.exe`. Phase 2 of
 * the ecosystem refactor forbids that — packages must load into WebView2 with
 * no Node runtime, so `process` is undefined there.
 *
...
/** Possible host platforms the Claude Code CLI argv selection understands. */
export type ClaudeCodeHostPlatform = 'win32' | 'linux' | 'darwin'

/** Default platform when no UA hint is available. */
const DEFAULT_PLATFORM: ClaudeCodeHostPlatform = 'linux'

/**
 * Detect the host platform for the Claude Code CLI argv selection.
 *
 ...
 * `claudeSpawnSpec` previously accepted from `process.platform`. The
 ...
 * touching callers.
 *
 * @returns the platform string in the shape `claudeSpawnSpec` expects.
 */
export async function detectHostPlatform(): Promise<ClaudeCodeHostPlatform> {

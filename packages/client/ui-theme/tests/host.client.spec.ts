...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    expect(ctx.settings.get(ns)).toEqual({ preference: DEFAULT_PREFERENCE, skin: DEFAULT_SKIN, background: '', backgroundName: '' })
    await ctx.settings.update(ns, { preference: 'dark' })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'dark', skin: DEFAULT_SKIN, background: '', backgroundName: '' })

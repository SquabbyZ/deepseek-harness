...
export interface AgentInventorySettingsTabInjected {
  list: () => Promise<{ entries: readonly AgentInventoryEntry[] }>
  refresh: () => void
  store: AgentInventoryStore
  setEnabled: (action: { entryId: string; enabled: boolean }, options: { signal: AbortSignal }) => Promise<void>
}

export type AgentInventorySettingsTabProps =
  PropsRuntime<'settings.plugins.tab'>
  & PropsLocale<'settings.agent'>
  & InjectFace<AgentInventorySettingsTabInjected>

type ViewState =
  | { readonly status: 'loading' }
  | { readonly status: 'error' }
  | { readonly status: 'ready'; readonly entries: readonly AgentInventoryEntryView[] }
...
export function AgentInventorySettingsTab({
  store,
  setEnabled,
  refresh,
  t,
}: AgentInventorySettingsTabProps): ReactNode {
  ...
  const filteredEntries = useMemo(() => {
    ...
    if (view.status !== 'ready') return [] as readonly AgentInventoryEntryView[]
    if (normalized.length === 0) return view.entries
    return view.entries.filter(entry =>
      [entry.presetId, entry.name, entry.description].some(value => value.toLocaleLowerCase().includes(normalized)),
    )
  }, [query, view])
  ...
  const { schedule: scheduleToggle, intendedSnapshot } = useDebouncedToggle<AgentEntryId>({
    debounceMs: 500,
      ...
      {view.status === 'ready' ? (
        <div className={CATALOG}>
          ...
          <div className={CATALOG_HEADING}>
            ...
            <span className={CATALOG_HEADING_COUNT} data-agent-count={filteredEntries.length}>
          ...
          {filteredEntries.length > 0 ? (
            <ul className={LIST}>
              {filteredEntries.map((entry) => {
                ...
                const sourceKey = `source${entry.source.charAt(0).toUpperCase()}${entry.source.slice(1).replace(/-./g, x => x[1].toUpperCase())}` as AgentInventoryLocaleKey
                const sourceLabel = t(sourceKey)
                const defaultLabel = entry.isDefault ? ` · ${t('defaultBadge')}` : ''
                return (
                  <li key={entry.entryId} data-agent-entry={entry.entryId}>
                    <SwitchRow
                      entryId={entry.entryId}
                      label={entry.name}
                      caption={`${sourceLabel}${defaultLabel}`}
                      checked={effective}

...
import { type McpInventoryEntryView, type McpInventoryStore } from './inventory-store.ts'
          ...
          {filteredEntries.length > 0 ? (
            <ul className={LIST}>
              {filteredEntries.map((entry) => {
                ...
                const transportKey = `transport${entry.transport.charAt(0).toUpperCase()}${entry.transport.slice(1).replace(/-./g, x => x[1].toUpperCase())}` as McpInventoryLocaleKey

...
describe('Menu', () => {
  ...
  it('selected item shows the trailing check; align=end, side=top, and className apply', () => {
    ...
    expect(menu.getAttribute('data-menu-side')).toBe('top')
    expect(menu.getAttribute('data-menu-align')).toBe('end')
  ...
  it('renders footer rows in a pinned section below the items; they still select', () => {
    ...
    expect((footerItem.closest('[data-menu-footer]'))).not.toBeNull()
    expect(screen.getByRole('menuitem', { name: 'Alpha' }).closest('[data-menu-footer]')).toBeNull()

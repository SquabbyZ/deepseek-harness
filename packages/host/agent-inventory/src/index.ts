...
function classifySource(preset: AgentPreset): AgentEntrySource {
  // System-trust presets ship with the deployment; surface them as 'bundled'
  // because the inventory schema's wire vocabulary enumerates the four user-
  // facing buckets ('bundled' | 'user' | 'project' | 'unknown') and rejects
  // any value outside the union, even though the runtime trust field has
  // more gradations.
  if (preset.trust === 'system') return 'bundled'
  if (preset.trust === 'user') return 'user'
  return 'unknown'
}

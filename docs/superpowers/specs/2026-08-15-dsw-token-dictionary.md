# DSW 设计令牌词典（迁移 shadcn/tailwind 的映射基础）

提取自 `packages/client/ui-theme/src/styles/*.css`，共 368 个 `--dsw-*` 引用（163 个核心定义 + 渐隐/滚动条/shiki 辅助）。
三层结构：`static`（原始色值）→ `alias`（语义角色，明暗两套）→ `specific`（组件级）。

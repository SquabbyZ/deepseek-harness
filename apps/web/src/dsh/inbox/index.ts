...
// TODO(phase3): re-enable per-plugin when vite-plugin-node-polyfills subpath bug is fixed.
// The full inbox barrel (116 packages) triggers Vite optimizeDeps to pre-bundle
// every plugin, which in turn pulls in `vite-plugin-node-polyfills`. That plugin
// has an internal subpath resolution error: `undici/lib/mock/snapshot-recorder.js`
// does `require('node:fs/promises')` from a path the polyfill can't satisfy.
// Result: `vite build` (and the dev pre-bundle) crash before the host ever
// boots. Until the upstream polyfill ships a fix, we ship an empty barrel
// and rely on the Phase 1 + Phase 2 UI install flow for plugin discovery.
//
// Once re-enabled, restore the commented imports below and the full
// `inboxPlugins` array, then bump `inboxPluginsCount` back to 116.
 ...
 *
 * Empty in Phase 2 follow-up #9 (see header note). Phase 1 + Phase 2's UI
 * install flow remains the supported discovery path.
 */
export const inboxPlugins = [] as const

/** Number of in-box plugins the shell boot wires into the host context. */
export const inboxPluginsCount = 0
